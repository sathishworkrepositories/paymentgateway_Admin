...
class BalanceMATICUpdate extends Command
{
    ...
    {    
        $useraddress = UserEvmAddress::where(['matic_status' => 0])->get();
        if(count($useraddress) > 0){            
			foreach ($useraddress as $user) {
				$uid = $user->user_id;
				$this->updateTransactionEVM('MATIC',$uid);
				sleep(1); // this should halt for 2 seconds for every loop
				UserEvmAddress::where(['id' => $user->id])->update(['matic_status' => 1,'updated_at' => date('Y-m-d H:i:s',time())]);
			}
		}else{
            $users = UserEvmAddress::where('matic_status',1)->orderBy('user_id','Desc')->get();
            foreach ($users as $user) {
                UserEvmAddress::where('user_id',$user->user_id)->update(['matic_status' => 0, 'updated_at' => date('Y-m-d H:i:s',time())]);
            }
        }
